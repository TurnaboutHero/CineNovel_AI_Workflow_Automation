...
## 임무
아래 아웃라인과 참조 자료를 바탕으로 {ep_tag} 본문을 작성하세요.

## 형식 요구사항
- 분량: {format.char_count[0]}~{format.char_count[1]}자
- 씬: {format.scenes[0]}~{format.scenes[1]}개
- [IMG] 마커: {format.img_markers[0]}~{format.img_markers[1]}개

## 아웃라인
{context.outline}

## 씬 비트시트 (있으면)
{context.plot_beats 또는 "plot-beats.md 미존재 — 자유 구성"}

## 직전 에피소드 엔딩 훅
{context.prev_ending_hook}

## 직전 에피소드 마지막 500자 (연속성)
...
**결과 저장:**
- Write로 `{project}/episodes/.staging/{ep_tag}_draft_v1.md`에 저장
...
| 초고 분량 심각 부족 (<7000자) | 즉시 재생성 (수정 아닌 새 초고) |
